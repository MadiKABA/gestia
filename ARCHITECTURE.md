...
**Un seul breakpoint desktop dans tout le projet : `lg` (1024px, défaut
Tailwind — aucune config custom dans `globals.css`).** Il gouverne à la fois
le chrome de l'app (`AppShell` : sidebar fixe vs bottom tab bar, voir
`sidebar-fixed.tsx`/`bottom-tab-bar.tsx`/`app-header.tsx`) et le contenu de
chaque écran (cards mobile vs tableau desktop). Historique : deux écrans
(`VendeursPanel`, `TransactionsList`) avaient basculé leur propre bloc
cards/tableau à `md` (768px) en se calquant l'un sur l'autre plutôt que sur
le chrome — entre 768 et 1024px, le contenu se croyait "desktop" pendant que
le chrome restait en mode mobile, rendu hybride incohérent. Corrigé, `lg`
partout désormais.

`md`/`sm` restent utilisables librement pour du **reflow interne à un même
design** (ex: un formulaire qui passe d'empilé à côte-à-côte, `sm:flex-row`)
— jamais pour le switch structurel mobile-cards ⇄ desktop-tableau, qui doit
toujours se faire sur `lg`.

**Pattern à deux blocs**, jamais de détection JS (`useMediaQuery`) pour ce
switch — l'exception documentée est `ResponsivePanel`
(`presentation/shared/components/responsive-panel.tsx`), qui change la
_nature_ du composant (`Dialog` vs `Sheet`), pas un simple bloc de layout ;
son hook est SSR-safe (`useSyncExternalStore`, snapshot serveur toujours
`false`) précisément parce qu'il ne sert qu'à ce cas étroit. Pour tout le
reste, deux blocs CSS parallèles, jamais un seul bloc conditionné par du JS :

```tsx
{
  /* Mobile (< lg) : cards simples. */
}
<ul className="grid grid-cols-1 gap-2 lg:hidden">...</ul>;
...
Référence canonique : `presentation/transaction/components/transactions-list.tsx`
— conteneur `mx-auto w-full max-w-md space-y-4 p-4 lg:max-w-5xl`, cartes
résumé 2 cases mobile / jusqu'à 4 desktop (`lg:hidden` /
`hidden lg:grid lg:grid-cols-4`), liste cards → tableau avec actions par
ligne (icônes `Button variant="ghost" size="icon-sm"`). Tout nouvel écran
liste (Party, CashMovement, futurs modules) suit ce même pattern — ne pas
ajouter de colonne Actions si aucune action (edit/delete/...) n'existe
réellement pour l'entité (`CaissePage` n'en a pas : aucun use-case
update/delete pour `CashMovement`).

### Contraintes Android/iOS

**Zones sûres (`env(safe-area-inset-*)`)** : tout élément `fixed` qui touche
un bord de l'écran doit compenser l'encoche/l'île dynamique (haut) ou la
barre de gestes (bas) sur iPhone. Deux façons distinctes selon le bord,
ne pas confondre : le bottom tab bar (`bottom-tab-bar.tsx`) ajoute l'inset en
**padding dans une hauteur fixe** (`h-16 pb-[env(safe-area-inset-bottom)]`) —
correct ici parce que ça recentre le contenu vers le haut, loin de la zone de
gestes, sans changer l'empreinte totale à l'écran. Le header (`app-header.tsx`)
fait l'inverse : **hauteur agrandie de l'inset**
(`h-[calc(3.5rem+env(safe-area-inset-top))] pt-[env(safe-area-inset-top)]`) —
un simple padding dans une hauteur fixe écraserait son contenu sous
l'encoche au lieu de le décaler en dessous. Le `<main>` de `app-shell.tsx`
doit rester synchronisé avec la hauteur réelle du header
(`pt-[calc(3.5rem+env(safe-area-inset-top))]`), jamais une valeur fixe qui
suppose l'inset à zéro.

**Background Sync / stockage persistant** : déjà documenté en détail plus
loin dans ce fichier, section "Limitations iOS — à ne jamais revisiter comme
un bug" (sous "Synchronisation descendante (pull)") — ne pas dupliquer ici.

**Clavier / `inputmode`** : pattern déjà en place, à reproduire pour tout
futur champ numérique ou code :

- PIN (`presentation/shared/components/pin-input.tsx`) : `InputOTP` avec
  `pattern={REGEXP_ONLY_DIGITS}`, `inputMode="numeric"`, slots `mask` (jamais
  affiché en clair).
- OTP (`presentation/shared/components/otp-input.tsx`) : même `InputOTP`,
  sans `mask` (doit rester lisible), `autoComplete="one-time-code"` par
  défaut de la librairie `input-otp` (déclenche l'auto-remplissage SMS sur
  Android et iOS).
- Montant (`transaction-create-form.tsx`) : `type="number"
inputMode="decimal"` — ouvre le clavier numérique adapté sur les deux
  plateformes.

Ces trois composants ont été audités pour cette convention : aucun écart
