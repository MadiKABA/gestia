 ...
 * l'erreur pour le seul cas "session expirée/absente" : les classes
 * d'erreur custom ne survivent pas à la sérialisation d'une Server Action
 * (seul `message` traverse), donc un `instanceof ForbiddenError` côté
 * client ne fonctionnerait pas — voir infrastructure/offline/sync-engine.ts.
 * Toute autre erreur (validation, bug serveur, réseau) continue de rejeter
