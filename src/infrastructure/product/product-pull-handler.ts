...
import {
  ...
  toDomainProduct,
} from "@/infrastructure/product/product.repository";
...
export const productPullHandler: PullHandler<Product> = {
  async findChangedSince(context, since, queryStartedAt, pageCursor) {
    ...
    const records: PulledRecord<Product>[] = page.map((row) => ({
      id: row.id,
      updatedAt: row.updatedAt.toISOString(),
      deletedAt: row.deletedAt?.toISOString() ?? null,
      data: toDomainProduct(row),
    }));
